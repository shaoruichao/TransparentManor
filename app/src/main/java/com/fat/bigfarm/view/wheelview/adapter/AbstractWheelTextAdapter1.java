 ...
 */
package com.fat.bigfarm.view.wheelview.adapter;
...
public abstract class AbstractWheelTextAdapter1 extends AbstractWheelAdapter {
	...
	public View getItem(int index, View convertView, ViewGroup parent) {
		if (index >= 0 && index < getItemsCount()) {
			...
			ImageView iv_temp = getImageView(convertView, R.id.iv_temp);
			...
			if (textView != null) {
				...
				if ("男".equals(text)){
					...
					iv_temp.setImageResource(R.drawable.button_sex_men_s);
				}else if ("女".equals(text)){
					iv_temp.setVisibility(View.VISIBLE);
					iv_temp.setImageResource(R.drawable.button_sex_women_s);
				}else if ("保密".equals(text)){
	...
	private ImageView getImageView(View view, int textResource) {
		...
		try {
			if (textResource == NO_RESOURCE && view instanceof ImageView) {
				...
				image = (ImageView) view.findViewById(textResource);
			}
		} catch (ClassCastException e) {
